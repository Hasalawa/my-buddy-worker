    ...
    <div className="w-full relative overflow-x-clip pb-10">
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="max-w-7xl mx-auto w-full space-y-8">
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-white">Job Management</h1>
            <p className="text-gray-400 mt-1 text-sm">Monitor ongoing work and moderate flagged listings.</p>
          </div>
          
          <div className="flex items-center gap-2 bg-gray-900/50 border border-gray-800 rounded-xl px-4 py-2 w-full sm:w-auto focus-within:border-brand-green transition-colors">
            <Search size={18} className="text-gray-500" />
            <input type="text" placeholder="Search Job ID or Title..." className="bg-transparent border-none outline-none text-sm w-full sm:w-64 placeholder:text-gray-600 text-white" />
          </div>
        </motion.div>
        ...
        {/* Tab Content Area */}
        <motion.div variants={itemVariants} className="bg-[#111111]/80 border border-gray-800/80 rounded-2xl p-5 sm:p-6 backdrop-blur-sm shadow-xl min-h-[500px]">
          <AnimatePresence mode="wait">
                ...
                <div className="overflow-x-auto custom-scrollbar pb-4">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-gray-800/50">
                      {activeJobs.map(job => (
                        <tr key={job.id} className="hover:bg-gray-900/30 transition-colors group">
                          <td className="py-4 px-4">
                            <p className="font-semibold text-white group-hover:text-brand-green transition-colors">{job.title}</p>
                            <span className="text-xs text-gray-500">ID: {job.id}</span>
                          </td>
                          <td className="py-4 px-4">
                            <div className="flex flex-col gap-1">
                              <span className="text-xs text-gray-400">Buyer: <span className="text-white">{job.buyer}</span></span>
                              <span className="text-xs text-gray-400">Worker: <span className={job.worker === 'Pending' ? 'text-orange-400' : 'text-white'}>{job.worker}</span></span>
                            </div>
                          </td>
                          <td className="py-4 px-4 font-semibold text-white">{job.amount}</td>
                          <td className="py-4 px-4">
                            <div className="flex flex-col items-start gap-1.5">
                              <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${
                                job.status === 'Open' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-brand-green/10 text-brand-green border-brand-green/20'
                              }`}>
                                {job.status}
                              </span>
                              <span className="text-[11px] text-gray-500 flex items-center gap-1"><Clock size={10} /> {job.deadline}</span>
                            </div>
                          </td>
                          <td className="py-4 px-4 text-right">
                            <button className="p-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-gray-300 transition-colors tooltip-trigger" title="View Details">
                              <Eye size={16} />
                            </button>
                          </td>
                        </tr>
            ...
            {activeTab === 'flagged' && (
              <motion.div key="flagged" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-6">
                <div className="flex justify-between items-center border-b border-gray-800/50 pb-4">
                  <h3 className="text-lg font-semibold text-white">Reported Listings</h3>
                  <span className="text-sm text-red-400 font-medium flex items-center gap-2"><AlertTriangle size={16} /> Action Required</span>
                </div>
                
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {flaggedContent.map(item => (
                    <div key={item.id} className="bg-gray-900/50 border border-gray-800 rounded-xl p-5 relative overflow-hidden group">
                      {/* Risk Indicator line */}
                      <div className={`absolute top-0 left-0 w-1 h-full ${item.risk === 'High' ? 'bg-red-500' : 'bg-orange-500'}`} />
                      
                      <div className="pl-3">
                        <div className="flex justify-between items-start mb-2">
                          <span className="px-2 py-1 bg-gray-800 text-gray-300 text-[10px] font-mono rounded-md border border-gray-700">ID: {item.id}</span>
                          <span className="text-xs text-gray-500">{item.date}</span>
                        </div>
                        
                        <h4 className="font-semibold text-white mb-4 line-clamp-2">{item.title || item.jobTitle}</h4>
                        
                        <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 mb-5">
                          <p className="text-xs text-red-400 font-medium mb-1 flex items-center gap-1.5"><ShieldAlert size={14} /> Reason for Report:</p>
                          <p className="text-sm text-white">{item.reason}</p>
                          <p className="text-[11px] text-gray-500 mt-2">Reported by: {item.reportedBy}</p>
                        </div>

                        <div className="flex gap-3">
                          <button className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-lg transition-colors text-xs sm:text-sm font-bold">
            ...
            {/* ---------------- COMPLETED JOBS TAB ---------------- */}
            {activeTab === 'completed' && (
              <motion.div key="completed" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="flex flex-col items-center justify-center h-64 text-gray-500">
                <CheckCircle2 size={48} className="text-gray-800 mb-4" />
